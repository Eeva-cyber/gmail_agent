...
# Initialize Supabase client (do this once, e.g., in your main application setup)
supabase_url = os.getenv("DATABASE_URL", "")
supabase_key = os.getenv("DATABASE_API_KEY", "")
supabase_client: Client = create_client(supabase_url, supabase_key)

def store_message(user_data, message_data):
    """
    ...
    try:
        ...
        message_response = supabase_client.table("messages") \
...
if __name__ == "__main__":
    # Example data you would parse from the Gmail API and your agent
    example_user = {
    ...
    example_message = {
        "thread_id": "18a9f84f0c5d7bae",
        "message_id": "CAHkZjsD0eXAmpf",
        "sender": "user",  # Changed from email address to ENUM value
        "body": "Hey I studying Science hoping to major in Computer Science...",
        "subject": "Re: Welcome to RAID!",
        "timestamp": "2024-01-15T10:32:00Z" # Must be ISO 8601 format
    }
    # Call the function to store the data
    success, error = store_message(example_user, example_message)
    if not success:
